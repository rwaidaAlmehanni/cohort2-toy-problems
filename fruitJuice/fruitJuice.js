...
Jar.prototype.getConcentration=function(fruit){
	var sum=0
	if(this.fruitIn[fruit]===undefined)return 0;
	for(var k in this.fruitIn){
		sum+=this.fruitIn[k];
	}
     return (this.fruitIn[fruit]/sum)*100+"%";
	}

Jar.prototype.add=function(amount,fruit){
	if(this.fruitIn[fruit]!==undefined){
		this.fruitIn[fruit]+=amount;
	}
	else{this.fruitIn[fruit]=amount;}
}
